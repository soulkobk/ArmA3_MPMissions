/*
	...
	Description: allows users spectator access (via O key). playerUID must be in array OWP_var_whiteListedSpectate
...
OWP_fncl_addLocations = {
	_locationIcon = "\A3\3den\Data\Displays\Display3DEN\PanelLeft\locationList_ca.paa";
	_locationMarkerArray = allMapMarkers select {(["OWP_mkr_",_x] call BIS_fnc_inString) isEqualTo true};
	{
		_locationMarker = _x;
...
OWP_fncl_spectate = {
	...
	if (_isSpectating isEqualTo "true") then
	{
		...
		// [] spawn {call OWP_fncl_addLocations};
	...
	if (_isSpectating isEqualTo "false") then
	{
		...
		[] spawn {call OWP_fncl_deleteLocations};
...
waitUntil {!isNil "OWP_var_whiteListedSpectate"};
waitUntil {!isNil "OWP_var_spectatorInit"};

_playerUID = getPlayerUID player;
_isWhiteListed = _playerUID in OWP_var_whiteListedSpectate;
