/*
	...
	Name: SGC_fnc_zeusInitServer.sqf
	Version: 1.0
	...
	initPlayerLocal.sqf, add the below code to it.
		[] spawn
		{
			waitUntil {!isNil "SGC_var_whiteListedZeus"};
			waitUntil {!isNil "SGC_var_zeusInit"};
...
waitUntil {!(SGC_var_whiteListedZeus isEqualTo [])};
...
_i = 0;
{
	...
	_zeusCurator addEventHandler ["CuratorObjectPlaced",{_this remoteExecCall ["SGC_fnc_zeusAddObject"];}];
	_zeusCurator addEventHandler ["CuratorObjectDeleted",{_this remoteExecCall ["SGC_fnc_zeusDeleteObject"];}];

} forEach SGC_var_whiteListedZeus;
...
SGC_var_zeusInit = true;
publicVariable "SGC_var_zeusInit";
