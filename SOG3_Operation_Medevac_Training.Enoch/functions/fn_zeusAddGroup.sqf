...
{
	diag_log format ["[ZEUS DEBUG] %1 placed group %2",_nameOfCurator,_placed];
	...
	{
		[_placed] call KOBK_fnc_aiSetSkill;
		_placed addEventHandler ["KILLED", {(_this select 0) spawn KOBK_fnc_corpseCleanupHandler}];
		[_placed] spawn KOBK_fnc_unitEventHandler;
	};
