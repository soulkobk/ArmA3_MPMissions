...
{
	_unitObject = _x;
	_unitEventHandlerAlreadySet = _unitObject getVariable ["KOBK_var_unitEventHandlers",false];
	if (_unitEventHandlerAlreadySet isEqualTo false) then
	{
		_unitObject setVariable ["KOBK_var_unitEventHandlers",true,true]; // so we dont double up on event handlers if already set
