/*
	...
	Description: allows users spectator access (via O key). playerUID must be in array SGC_var_whiteListedSpectate
...
SGC_fncl_addLocations = {
	_locationIcon = "\A3\3den\Data\Displays\Display3DEN\PanelLeft\locationList_ca.paa";
	_locationMarkerArray = allMapMarkers select {(["SGC_mkr_",_x] call BIS_fnc_inString) isEqualTo true};
	{
		_locationMarker = _x;
...
SGC_fncl_spectate = {
	...
	if (_isSpectating isEqualTo "true") then
	{
		...
		// [] spawn {call SGC_fncl_addLocations};
	...
	if (_isSpectating isEqualTo "false") then
	{
		...
		[] spawn {call SGC_fncl_deleteLocations};
...
waitUntil {!isNil "SGC_var_whiteListedSpectate"};
waitUntil {!isNil "SGC_var_spectatorInit"};

_playerUID = getPlayerUID player;
_isWhiteListed = _playerUID in SGC_var_whiteListedSpectate;
