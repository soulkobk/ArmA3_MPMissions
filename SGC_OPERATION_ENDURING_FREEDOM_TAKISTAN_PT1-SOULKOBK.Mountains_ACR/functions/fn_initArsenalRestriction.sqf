...
[missionNamespace, "arsenalClosed", {
	...
	if !(SGC_arr_allowedHeadgear isEqualTo []) then
	{
		if !(_headgear in SGC_arr_allowedHeadgear) then
		{
			removeHeadgear player;
			player addHeadgear selectRandom SGC_arr_allowedHeadgear;
			_outputText = _outputText + "headgear ";
		};
	};
	if !(SGC_arr_allowedUniforms isEqualTo []) then
	{
		if !(_uniform in SGC_arr_allowedUniforms) then
		{
			_uniformItems = uniformItems player;
			removeUniform player;
			player forceAddUniform selectRandom SGC_arr_allowedUniforms;
			{player addItemToUniform _x} forEach _uniformItems;
			_outputText = _outputText + "uniform ";
		};
	};

	if !(SGC_arr_allowedVests isEqualTo []) then
	{
		if !(_vest in SGC_arr_allowedVests) then
		{
			_vestItems = vestItems player;
			removeVest player;
			player addVest selectRandom SGC_arr_allowedVests;
			{player addItemToVest _x} forEach _vestItems;
			_outputText = _outputText + "vest ";
		};
	};

	if !(SGC_arr_allowedBackpacks isEqualTo []) then
	{
		if !(_backpack in SGC_arr_allowedBackpacks) then
		{
			_backpackItems = backpackItems player;
			removeBackpack player;
			player addBackPack selectRandom SGC_arr_allowedBackpacks;
