/*
	...
	or place this in your initPlayerLocal.sqf/onPlayerRespawn.sqf to enable all players auto chute...
...
[_unit] spawn
{
	...
	while {true} do
	{
		...
		deployChuteAction = _unit addAction ["Deploy Parachute",{deployChute = true},[],9,true,true,"","true",5,false,"",""];  
		deployChute = false;
		while {deployChute isEqualto false} do
		{
			uiSleep 0.5;
			if !(vehicle _unit isEqualto _unit) exitWith {deployChute = false};
			if !(alive _unit) exitWith {deployChute = false};
			if ((position _unit select 2) <= 100) then {deployChute = true};
		};
		if (deployChute isEqualto true) then
		{
			addCamShake [8, 2, 20];
			...
			_unit removeAction deployChuteAction;
		};
	};
};
