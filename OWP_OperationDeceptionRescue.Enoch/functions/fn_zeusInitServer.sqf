/*
	...
	Name: KOBK_fnc_zeusInitServer.sqf
	Version: 1.0
	...
	initPlayerLocal.sqf, add the below code to it.
		[] spawn
		{
			waitUntil {!isNil "KOBK_var_whiteListedZeus"};
			waitUntil {!isNil "KOBK_var_zeusInit"};
	...
	onPlayerRespawn.sqf, add the below code to it.
		[] spawn
		{
			if ((!isNil "KOBK_var_whiteListedZeus") && (!isNil "KOBK_var_zeusInit")) then
			{
				_curatorArr = allCurators select {((_x getVariable ["UID","0"]) isEqualTo (getPlayerUID player))};
				if !(_curatorArr isEqualTo []) then
				{
...
// waitUntil {!(KOBK_var_whiteListedZeus isEqualTo [])};
waitUntil {!isNil "KOBK_var_whiteListedZeus"};
...
_i = 0;
{
	...
	_zeusCurator addEventHandler ["CuratorObjectPlaced",{params ["_curator","_placed"]; [_curator,_placed] remoteExecCall ["KOBK_fnc_zeusAddObject",[0,-2] select isDedicated];}];
	_zeusCurator addEventHandler ["CuratorObjectDeleted",{params ["_curator","_deleted"]; [_curator,_deleted] remoteExecCall ["KOBK_fnc_zeusDeleteObject",[0,-2] select isDedicated];}];

} forEach KOBK_var_whiteListedZeus;

_zeusCuratorVehicles = (nearestObjects [[0,0,0], ["AllVehicles"], 100000]);
{
	_zeusCurator = _x;
	_zeusCurator addCuratorEditableObjects [_zeusCuratorVehicles,true];
} forEach allCurators;

KOBK_var_zeusInit = true;
publicVariable "KOBK_var_zeusInit";
