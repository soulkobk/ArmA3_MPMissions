/*
	...
	["sgc", {[[(_this select 0),player]] remoteExec ["SGC_fnc_chatCommands",2]}, "admin"] call CBA_fnc_registerChatCommand;
...
if !(_playerOwnerUID in SGC_var_whiteListedChatCommands) exitWith {}; // allow access only to those that are in the white list.
...
switch _command do
{
	case "skiptime": {
		...
		{
			params ["_commandVar"];
			missionNamespace setVariable ["SGC_var_skiptime",_commandVar,true];
			[[],{titleText [format ["<t color='#ffffff' size='4'>SKIPPING TIME %1 HOUR(S)</t>",missionNamespace getVariable ["SGC_var_skiptime","NIL"]], "BLACK OUT", 2, true, true]}] remoteExec ["call",0];
			sleep 2;
			[(parseNumber _commandVar)] remoteExec ["skipTime"];
			sleep 2;
			[[],{titleText [format ["<t color='#ffffff' size='4'>SKIPPING TIME %1 HOUR(S)</t>",missionNamespace getVariable ["SGC_var_skiptime","NIL"]], "BLACK IN", 2, true, true]}] remoteExec ["call",0];
		};
	};
	case "warn": {
	...
	case "kill": {
		...
		if !(_playerArr isEqualTo []) then
		{
			...
			if !(_clientOwner isEqualTo "") then
			{
				[_player,1] remoteExec ["setDammage",_clientOwner];
			};
		};
	};
	case "launch": {
	...
	case "stats": {
		...
		format ["[STATS] processes -> spawned %1 - execVM %2 - exec %3 - execFSM %4 - total %5",_spawnedRunning,_execVMRunning,_execRunning,_execFSMRunning,_total] remoteExec ["systemChat",[0,-2] select isDedicated];
		_unitsAlivePlayer = count (allPlayers select {(alive _x) && (simulationEnabled _x)});
		_unitsDeadPlayer = missionNamespace getVariable ["SGC_var_unitsDeadPlayer",0];
		_unitsAliveEnemy = count ((allUnits - allPlayers) select {(alive _x) && (simulationEnabled _x) && (side _x isEqualTo EAST)});
		_unitsDeadEnemy = missionNamespace getVariable ["SGC_var_unitsDeadEnemy",0];
		_unitsAliveCivilian = count ((allUnits - allPlayers) select {(alive _x) && (simulationEnabled _x) && (side _x isEqualTo CIVILIAN)});
		_unitsDeadCivilian = missionNamespace getVariable ["SGC_var_unitsDeadCivilian",0];
		_unitsAliveAnimal = count ((allUnits - allPlayers) select {(alive _x) && (simulationEnabled _x) && ((_x isKindOf "Animal") isEqualTo true)});
		_unitsDeadAnimal = missionNamespace getVariable ["SGC_var_unitsDeadAnimal",0];
