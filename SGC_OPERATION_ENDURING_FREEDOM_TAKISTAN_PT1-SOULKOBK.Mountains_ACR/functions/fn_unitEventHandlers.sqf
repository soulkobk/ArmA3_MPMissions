...
{
	_unitObject = _x;
	_unitEventHandlerAlreadySet = _unitObject getVariable ["SGC_var_unitEventHandlers",false];
	if (_unitEventHandlerAlreadySet isEqualTo false) then
	{
		_unitObject setVariable ["SGC_var_unitEventHandlers",true,true]; // so we dont double up on event handlers if already set
		
		// used for ace unconscious
		_unitObject addEventHandler ["Hit", {
			params ["_unit", "_source", "_damage", "_instigator"];
			_unit setVariable ["SGC_var_hit",_instigator,true];
		}];
		...
		_unitObject addEventHandler ["Killed", {
			...
			if (isNull _instigator) then {_instigator = _unit getVariable ["SGC_var_hit",objNull]};
			...
			{
				_deathCount = missionNamespace getVariable ["SGC_var_unitsDeadCivilian",0];
				missionNamespace setVariable ["SGC_var_unitsDeadCivilian",(_deathCount + 1),true];
			...
			{
				_deathCount = missionNamespace getVariable ["SGC_var_unitsDeadEnemy",0];
				missionNamespace setVariable ["SGC_var_unitsDeadEnemy",(_deathCount + 1),true];
			};
		}];
	};
